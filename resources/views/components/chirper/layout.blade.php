<!DOCTYPE html>
<html lang="en" data-theme="laravelChirper"> {{-- laravelChirper custom daisyui theme --}}
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>{{ isset($headTitle) ? $headTitle . ' - Chirper' : 'Chirper' }} | Laravel Bootcamp</title>
    {{-- SEO --}}
    <meta name="description" content="A simple Twitter clone built with Laravel and Tailwind CSS">
    <meta property="og:image" content={{ asset('chirper/images/og.jpeg') }}>
    ...
    <meta property="og:url" content="https://chirper.test">
    {{-- Favicons --}}
    <link rel="icon" type="image/x-icon" href="{{ asset('chirper/icons/favicon.ico') }}">
    <link rel="icon" type="image/svg+xml" href="{{ asset('chirper/icons/favicon.svg') }}">
    <link rel="apple-touch-icon" sizes="180x180" href="{{ asset('chirper/icons/apple-touch-icon.png') }}">
    <link rel="icon" type="image/png" sizes="96x96" href="{{ asset('chirper/icons/favicon-96x96.png') }}">
    {{-- Manifest --}}
    <link rel="manifest" href="{{ asset('chirper/site.webmanifest') }}">
    {{-- Fonts & theming --}}
    ...
    {{-- App resources --}}
    @vite(['resources/css/chirper.css'])
</head>
...
<nav class="navbar bg-base-100">
    ...
    <div class="navbar-end gap-2">
        @auth
            ...
            {{-- Logout button --}}
            <form method="POST" action="{{ route('chirps.logout') }}" class="inline">
                @csrf
                <button type="submit" class="btn btn-ghost btn-sm">Logout</button>
            </form>
        @else
            {{-- Login and register buttons --}}
            <a href="{{ route('chirps.login') }}" class="btn btn-ghost btn-sm">Sign In</a>
            <a href="{{ route('chirps.register.post') }}" class="btn btn-primary btn-sm">Sign Up</a>
        @endauth
    </div>
</nav>
...
<footer class="footer footer-center p-5 bg-base-300 text-base-content text-xs">
    <div>
        <p>© 2026 Chirper - Built by <a href="https://github.com/voorhof" target="_blank">Voorhof</a> with Laravel and ❤️</p>
    </div>
</footer>
</body>
</html>
